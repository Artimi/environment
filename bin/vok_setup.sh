#!/bin/sh
# script to add vok keyboard layout to system, need root


VOK_PATH='/usr/share/X11/xkb/symbols/vok'
XORG_LST_PATH='/usr/share/X11/xkb/rules/xorg.lst'
    ...
    <layout>
      <configItem>
       <name>vok</name>
        ...
        <description>vogo keyboard</description>
...
`
tmp_file=`mktemp`
echo "$vok_layout" > $tmp_file

echo "$vok" > $VOK_PATH
echo "Content of vok file copied to " $VOK_PATH

sed  -i '/! layout/ a vok   vogo keyboard' $XORG_LST_PATH
echo "Added line vok vogo keyboard to "$XORG_LST_PATH

sed -i "/<layoutList>/ r $tmp_file" $EVDEV_PATH
echo "Added layout to layoutList in "$EVDEV_PATH
